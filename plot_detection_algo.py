...
def plot(axis, hist_wt, hist_nt, inv_cumulative_wt, inv_cumulative_nt, legends, algo_name):
    ...
    for i in range(len(axis)):
        ax[0, 0].plot(hist_wt[i][1][1:], hist_wt[i][0], label = f'{legends[i]}_WT', color=colors[i], linewidth=i%3+1)
        ax[0,0].plot(hist_nt[i][1][1:], hist_nt[i][0], '--', label=f'{legends[i]}_NT', color=colors[i], linewidth=i%3+1)
        title1 += legends[i]
        ax[0, 0].grid()
        ax[0, 0].legend()

        ax[0, 1].plot(hist_wt[i][1][1:], np.log10(hist_wt[i][0]),label=f'{legends[i]}_WT', color=colors[i], linewidth=i%3+1)
        ax[0, 1].plot(hist_nt[i][1][1:], np.log10(hist_nt[i][0]), '--', label=f'{legends[i]}_NT', color=colors[i], linewidth=i%3+1)
        title2 += legends[i]
        ax[0, 1].grid()
        ax[0, 1].legend()

        ax[1, 0].plot(axis[i], inv_cumulative_wt[i], label=f'{legends[i]}_WT', color=colors[i], linewidth=i%3+1)
        ax[1, 0].plot(axis[i], inv_cumulative_nt[i], '--', label=f'{legends[i]}_NT', color=colors[i], linewidth=i%3+1)
        title3 += legends[i]
        ax[1, 0].grid()
        ax[1, 0].legend()

        ax[1, 1].plot(inv_cumulative_nt[i], inv_cumulative_wt[i], label=legends[i], color=colors[i], linewidth=i%3+1)
        title4 += legends[i]
        ax[1, 1].grid()
        ax[1, 1].legend()
